<script>
	...
	import { process_example } from '../examples.js';
	import InputOutputToggle from './InputOutputToggle.svelte';

	export let baseUrl = 'https://suaveio.web.app';
	...
	onMount(() => {
		...
		if (gist) {
			fetch(`${baseUrl}/repl/${gist}.json`).then(r => r.json()).then(data => {
			...
			fetch(`${baseUrl}/examples/${example}.json`).then(async response => {
				if (response.ok) {
